...
typedef enum {
    ...
    LUMP_CMD_MODES    = 0x01,
    LUMP_CMD_SPEED    = 0x02,
    LUMP_CMD_SELECT   = 0x03,
    LUMP_CMD_WRITE    = 0x04,
    LUMP_CMD_EXT_MODE = 0x06,
    LUMP_CMD_VERSION  = 0x07
} lump_cmd_t;
...
typedef enum {
    LUMP_INFO_NAME        = 0x00,    // INFO command - NAME    (device name)
    LUMP_INFO_RAW         = 0x01,    // INFO command - RAW     (device RAW value span)
    LUMP_INFO_PCT         = 0x02,    // INFO command - PCT     (device PCT value span)
    LUMP_INFO_SI          = 0x03,    // INFO command - SI      (device SI  value span)
    LUMP_INFO_UNITS       = 0x04,    // INFO command - UNITS   (device SI  unit symbol)
    LUMP_INFO_MAPPING     = 0x05,    // INFO command - MAPPING (input/output value type flags)
    LUMP_INFO_MODE_COMBOS = 0x06,    // INFO command - COMBOS  (mode combinations - LPF2-only)
    LUMP_INFO_MODE_PLUS_8 = 0x20,    // Bit flag used in powered up devices to indicate that the mode is 8 + the mode specified in the first byte
    LUMP_INFO_FORMAT      = 0x80     // INFO command - FORMAT  (device data sets and format)
} lump_info_t;

typedef enum {
// CMD_EXT_MODE payload
    LUMP_EXT_MODE_0 = 0x00,    // mode is < 8
    LUMP_EXT_MODE_8 = 0x08     // mode is >= 8
} lump_ext_mode_t;
